...
import { createVisualEditorWindow } from './windowManager'
...
export function sanitizeRoomNameToken(roomName: string): string {
  ...
  if (/[\\/:*?"<>|]/.test(raw) || raw.split('').some((c) => c.charCodeAt(0) <= 0x1f)) return ''
  return raw
}
...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle('visualEditor.open', async (_event, nextState: VisualEditorBridgeState | null) => {
    appState.latestVisualEditorState = nextState
  ...
  ipcMain.handle(
    ...
    async (_event, nextState: VisualEditorBridgeState | null) => {
      appState.latestVisualEditorState = nextState
      if (
        appState.visualEditorWindowRef &&
        !appState.visualEditorWindowRef.isDestroyed() &&
        nextState
      ) {
        appState.visualEditorWindowRef.webContents.send('visualEditor.stateUpdated', nextState)
      }
      return { synced: true }
    }
  )
  ...
  ipcMain.handle('visualEditor.getState', async () => {
    return appState.latestVisualEditorState
  })
  ...
  ipcMain.handle(
    ...
    async (_event, points: Array<{ x: number; y: number }> | null | undefined) => {
      ...
      if (appState.mainWindowRef && !appState.mainWindowRef.isDestroyed()) {
        appState.mainWindowRef.webContents.send('visualEditor.importPath', safePoints)
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (appState.mainWindowRef && !appState.mainWindowRef.isDestroyed()) {
        appState.mainWindowRef.webContents.send('visualEditor.importActors', safeActors)
        appState.mainWindowRef.focus()
      }
      return { imported: true }
    }
