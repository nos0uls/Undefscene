...
export function validateGraph(
  ...
  function contStateKey(nodeId: string, s: ContState): string {
    ...
    return `${nodeId}|${c}|${d}|${s.cameraOverridden ? 1 : 0}`
  }

  // Explore a parallel branch until stopNodeId, collecting final states.
  ...
  function dfsContinuity(nodeId: string, state: ContState, pathVisited: Set<string>) {
    ...
    if (node.type === 'parallel_start') {
      ...
      if (joinId && nodeMap.has(joinId)) {
        ...
        // Invalid parallel_start: explore outgoing edges normally
        const newPathVisited = new Set(pathVisited).add(nodeId)
        for (const edge of outs) {
          if (edge.conditionEnabled && edge.conditionIfFalse === 'wait_until_true') {
            const timeout = edge.endTimeoutSeconds
  ...
  if (startNode) {
    dfsContinuity(startNode.id, { created: new Set(), destroyed: new Set(), cameraOverridden: false }, new Set())
  }

  // Emit actorUsedBeforeCreate errors
  ...
  // Emit actorUsedAfterDestroy errors
  ...
  // Emit cameraOverrideNotReset tip
